...
import ImageUtilities from '../../Lib/ImageUtils'
...
class UserPage extends Component {
  ...
  _renderContent () {
    if (!this.state.username) {
      ...
      const {
        username, signature, company, position, avatarURI //, github, twitter, dribbble, instagram
      } = this.state
  ...
  _loadUser () {
    ...
    V2exApi.getPage(`/member/${username}`)
      .then($ => {
        ...
        const num = Number(StringUtils.matchFirst(_numberAndDateElementText, /第 (\d+) 号会员/))
        let regDate = StringUtils.matchFirst(_numberAndDateElementText, /(\d{4}-\d{2}-\d{2})/)
        if (regDate) {
          regDate = regDate.replace(/(-0)|(-)/g, '.')
        }
        const liveness = Number(StringUtils.matchFirst(_numberAndDateElementText, /今日活跃度排名 (\d+)/))
