...
class NewTopicPage extends Component {
  ...
  setUpNode = () => {
    ...
    if (nodeSlug) {
      V2exApi.getNodes().then(nodes => {
        ...
        Alert.alert('Error getting nodes:' + error)
      })
    }
  };
  ...
  postNewTopic = () => {
    ...
    const once = V2exApi.getOnce()
    this.onPosting()
    V2exApi.pos(`/new/${nodeSlug}`, { title, content, syntax: 0, once }).then($ => {
      this.onPostingEnd()
      const docTitle = $('title').text()
      if (docTitle.indexOf(title) > -1) {
        const canonicalElement = $('link[rel="canonical"]')
        const newTopicID = Number(StringUtils.matchFirst(canonicalElement.attr('href'), /t\/(\d+)/))
